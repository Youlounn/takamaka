<?php 
	...
	$req = $db->prepare('SELECT * FROM corbeillecampagne WHERE id = ?');
	$req->execute(array($idCampagne));
	if($campagne = $req->fetch())
	{
//Insertion dans la table campagne	
		$req = $db->prepare("INSERT INTO campagne set id = ?, titre = ?, nom = ?, date = ?, adrRep = ?, adrExp = ?, type = ?, id_message = ?, 
			id_user = ?");
		...
		///Suppression dans la table corbeillecampagne			
		$req = $db->prepare('DELETE FROM corbeillecampagne WHERE id = ?');
		$req->execute(array($idCampagne));
